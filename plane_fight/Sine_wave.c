...
void TimerIntInitial(void)
{
 ...
 void DrawPlane(x,y){
        ...
        for(i=y;i<y+60;i++)
        {
            for(j=x;j<x+48;j++)
            {
                temp=gImage_plane_6060[m];
                if(temp != 0X00){
                temp |= gImage_plane_6060[m+1]<<8;
 ...
 //画敌方飞机
  void DrawEnemy(x,y){
         ...
         for(i=y;i<y+60;i++)
         {
             for(j=x;j<x+41;j++)
             {
                 temp=gImage_Enemy6060[m];
                 if(temp!=0X00){
                 temp |= gImage_Enemy6060[m+1]<<8;
                 TFTLCD_DrawPoint( j, i,temp);
                 }
                 m++;
                 m++;
  ...
  void Timer0AIntHandler(void){
    ...
    if(start==1)
    {
     ...
     if(y_stone<340)
     {
        if(flag_stone==1)
        {
            ClearEnemy(x_stone,y_stone);
            DrawEnemy(x_stone,y_stone+=30);
        }
     }
     else
     {
